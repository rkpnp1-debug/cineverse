...
export default async function TVPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  let show: any;
  try { show = await getTV(Number(id)); } catch { notFound(); }
        ...
        <div className="flex flex-col md:flex-row gap-8">
          <div className="shrink-0 mx-auto md:mx-0">
            <div className="relative w-48 sm:w-56 aspect-[2/3] rounded-2xl overflow-hidden shadow-2xl ring-1 ring-white/10">
              <Image src={posterUrl(show.poster_path, "w500")} alt={show.name} fill className="object-cover" sizes="224px" />
            </div>
          </div>
          <div className="flex-1 pt-4">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3 text-sm">
              ...
              <span className="flex items-center gap-1 text-zinc-400"><Calendar className="h-4 w-4" />{formatDate(show.first_air_date)}</span>
              <span className="text-zinc-400">{show.number_of_seasons} Seasons · {show.number_of_episodes} Episodes</span>
              {show.status && <span className="rounded bg-white/10 px-2 py-0.5 text-xs uppercase">{show.status}</span>}
            </div>
            {show.genres?.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-2">
                {show.genres.map((g: any) => (
                  <span key={g.id} className="rounded-full border border-white/10 px-3 py-1 text-xs">{g.name}</span>
                ))}
              </div>
            )}
              ...
              <div className="flex gap-4 pb-2">
                {show.credits.cast.slice(0, 16).map((p: any) => (
                  <Link key={p.id} href={`/person/${p.id}`} className="w-28 shrink-0 group">
                    ...
                    <p className="mt-1.5 text-sm font-medium line-clamp-1 group-hover:text-cinema-400">{p.name}</p>
