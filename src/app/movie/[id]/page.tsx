...
import { Star, Clock, Calendar, DollarSign, ExternalLink, Play } from "lucide-react";
...
export default async function MoviePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  let movie: any;
  try { movie = await getMovie(Number(id)); } catch { notFound(); }
        ...
        <div className="flex flex-col md:flex-row gap-8">
          <div className="shrink-0 mx-auto md:mx-0">
            <div className="relative w-48 sm:w-56 aspect-[2/3] rounded-2xl overflow-hidden shadow-2xl ring-1 ring-white/10">
              <Image src={posterUrl(movie.poster_path, "w500")} alt={movie.title} fill className="object-cover" sizes="224px" />
            </div>
          </div>
          <div className="flex-1 pt-4">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3 text-sm">
              ...
              <span className="flex items-center gap-1 text-zinc-400"><Calendar className="h-4 w-4" />{formatDate(movie.release_date)}</span>
              <span className="flex items-center gap-1 text-zinc-400"><Clock className="h-4 w-4" />{formatRuntime(movie.runtime)}</span>
              {movie.status && <span className="rounded bg-white/10 px-2 py-0.5 text-xs uppercase">{movie.status}</span>}
            </div>
            {movie.genres?.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-2">
                {movie.genres.map((g: any) => (
                  <span key={g.id} className="rounded-full border border-white/10 px-3 py-1 text-xs text-zinc-300">{g.name}</span>
                ))}
              </div>
            )}
              ...
              {director && (
                <div>
                  ...
                  <p className="text-white font-medium">
                    <Link href={`/person/${director.id}`} className="hover:text-cinema-400">{director.name}</Link>
                  </p>
                </div>
              )}
              {movie.budget > 0 && (
                <div>
                  ...
                  <p className="text-white font-medium flex items-center gap-1"><DollarSign className="h-3.5 w-3.5" />{formatMoney(movie.budget)}</p>
                </div>
              )}
              {movie.revenue > 0 && (
                <div>
                  <span className="text-zinc-500">Revenue</span>
                  <p className="text-white font-medium flex items-center gap-1"><DollarSign className="h-3.5 w-3.5" />{formatMoney(movie.revenue)}</p>
                </div>
              )}
            </div>
            <div className="mt-6 flex flex-wrap gap-3">
              {trailer && (
                <a href={`https://www.youtube.com/watch?v=${trailer.key}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full bg-cinema-600 hover:bg-cinema-500 px-5 py-2.5 text-sm font-semibold text-white">
                  <Play className="h-4 w-4" />Trailer
                </a>
              )}
              {movie.imdb_id && (
                <a href={`https://www.imdb.com/title/${movie.imdb_id}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full bg-white/10 hover:bg-white/20 px-5 py-2.5 text-sm font-semibold">
                ...
                {movie.credits.cast.slice(0, 16).map((p: any) => (
                  <Link key={p.id} href={`/person/${p.id}`} className="w-28 shrink-0 group">
                    <div className="relative aspect-[2/3] rounded-xl overflow-hidden bg-surface-700">
                      <Image src={profileUrl(p.profile_path)} alt={p.name} fill className="object-cover group-hover:scale-105 transition" sizes="112px" />
                    </div>
                    <p className="mt-1.5 text-sm font-medium text-zinc-200 line-clamp-1 group-hover:text-cinema-400">{p.name}</p>
