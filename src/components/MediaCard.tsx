...
export function MediaCard({ item, type }: { item: Media; type?: "movie" | "tv" }) {
  ...
  return (
    <Link href={href} className="poster-card group block w-[140px] sm:w-[160px] shrink-0">
      <div className="relative aspect-[2/3] overflow-hidden rounded-xl bg-surface-700 shadow-lg">
        <Image src={posterUrl(item.poster_path)} alt={title} fill sizes="160px" className="object-cover transition-transform duration-300 group-hover:scale-105" />
        {item.vote_average > 0 && (
      ...
      <div className="mt-2">
        <h3 className="text-sm font-medium text-zinc-100 line-clamp-2 group-hover:text-cinema-400 transition-colors">{title}</h3>
